...
$link = get_field('link');
$btnName = get_field('btn_name');
$phones = @settings('phones');
?>
<div class="<?= $classes; ?> <?= $align; ?> products_block">
	<div class="container">
		<div class="block_head">
			...
			<?php } ?>
			<div class="arrows_holder" id="arrow_holder_<?=$block['id'] ?>">
		...
		<?php if ($products == true && !empty($products)) { ?>
			<div class="gallery_slider swiper archive_container custom" id="gallery_slider_<?= $block['id'] ?>">
		...
		<div class="products_block-wrap">
			<a href="<?= $link ?>" class="btn products_block-btn"><?= $btnName ?></a>
		</div>
	</div>
</div>
...
<div class="<?= $classes; ?> <?= $align; ?> products_block">
    ...
    <?php if ($products == true && !empty($products)) { ?>
			<div class="produtc_cont" style="background:url('<?=wp_get_attachment_image_url($gr, 'full')?>') #f1f1f3;">    <div class="container">
				<div class="block_head">
					<?php if ($title) { ?>
						<h2 class="block-title first_paramm"><?= $title ?></h2>
					<?php } ?>
				</div>
			</div>
            <div class="container swiper swiper-container" id="gall_slider_<?= $block['id'] ?>">
                    ...
                    <?php foreach ($products as $item) { ?>
                        <div class="product_item swiper-slide">
                            <?php
                                ...
                                $post_object = get_post($product->get_id());
                                setup_postdata($GLOBALS['post'] = &$post_object);
                                $title_pr = get_the_title($item);
                                $link_pr = get_permalink($item);
                                $image_pr = get_post_thumbnail_id($item);
                                $desc_pr = get_field('descc', $item->ID);
                            ?>
                            <li class="product type-product">
                                <a href="<?=$link_pr?>" class="woocommerce-LoopProduct-link woocommerce-loop-product__link">
                                    <div class="product-image">
                                        <?= wp_get_attachment_image($image_pr, 'full') ?>
                                    </div>
                                    <h2 class="woocommerce-loop-product__title"><?=$title_pr?></h2>
                                    <div class="product-wrap">
                                        <div class="product_desc">
                                            <?=$desc_pr?>
                                        </div>
                                        <button class="btn open-modal" data-modal="callback">Забронировать</button>
                                    </div>
                                </a>
                            </li>
                        </div>
			...
			<div class="bottom_text container"><?=$bottom_text?></div>
            <div class="nav" id="arrow_holder_<?=$block['id'] ?>">
                    <div class="nav__prev">
                        <svg width="16" height="10" viewBox="0 0 16 10" fill="none" xmlns="http://www.w3.org/2000/svg">
...
jQuery(document).ready(function($){
    var swiper_<?php echo $slider_id ?> = new Swiper("#gall_slider_<?php echo $slider_id ?>", {
        slidesPerView: 3,
        spaceBetween: 15,
        navigation: {
            nextEl: '#arrow_holder_<?php echo $slider_id ?> .nav__next',
            prevEl: '#arrow_holder_<?php echo $slider_id ?> .nav__prev',
        },
        autoplay: {
            delay: 5000,
        },
